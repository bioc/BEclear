...
#' function
#'
#' @details All genes with an absolute median difference >= 0.05 and a p-value
#' of <= 0.01 are summarized into a data.frame. These genes are assumed to
#' contain a batch effect, whether the batch is shifted up or down.
#'
...
#' documentation of this function.
#' @param mediansTreshold the threshold above or equal absolute median
#' differences are regarded as batch effected, when the criteria for p-values
#' is also met.
...
#' and "p-value" containing the calculated median difference values (with their
#' sign) and the p-values, respectively.
#'
#' @examples
...
calcSummary <- function(medians, pvalues, mediansTreshold = 0.05,
    ...
    ## find affected genes; a batch can be shifted up or down
    affected <- apply(x, c(1,2), FUN =
                          function(X){ return((is.na(X[1]) |  abs(X[1]) >= mediansTreshold)
                                   & X[2] <= pvaluesTreshold)})
