...
#' p-values less or equal to 0.01 and absolute median differences greater or
#' equal to 0.05.
...
#' absolute median difference value and a column with the calculated BEscore.
#' These found genes are assumed to be batch affected due to their difference in
...
#' The 11 columns containing the numbers of found genes count the absolute
#' median difference values which are ranging from >= 0.05 to < 0.1 ; >= 0.1 to
#' < 0.2; >= 0.2 to < 0.3 and so on up to a limit of 1.\cr
...
calcScore <- function(data, samples, summary, saveAsFile = FALSE, dir = getwd()) {
  ...
  ## count the medians by the size of the shift, in either direction
  DT <- lapply(batches, FUN = function(x, y){
      data.table(batch_id = x,
                 count05 = y[batch_id == x & abs(median) >= 0.05 & abs(median) < 0.1, .N],
                 count1 = y[batch_id == x & abs(median) >= 0.1  & abs(median) < 0.2,  .N],
                 count2 = y[batch_id == x & abs(median) >= 0.2  & abs(median) < 0.3,  .N],
                 count3 = y[batch_id == x & abs(median) >= 0.3  & abs(median) < 0.4,  .N],
                 count4 = y[batch_id == x & abs(median) >= 0.4  & abs(median) < 0.5,  .N],
                 count5 = y[batch_id == x & abs(median) >= 0.5  & abs(median) < 0.6,  .N],
                 count6 = y[batch_id == x & abs(median) >= 0.6  & abs(median) < 0.7,  .N],
                 count7 = y[batch_id == x & abs(median) >= 0.7  & abs(median) < 0.8,  .N],
                 count8 = y[batch_id == x & abs(median) >= 0.8  & abs(median) < 0.9,  .N],
                 count9 = y[batch_id == x & abs(median) >= 0.9,  .N])}, summary)
